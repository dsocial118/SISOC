    ...
    def get_context_data(self, **kwargs):
        ...
        if query:
            object_list = LegajosDerivaciones.objects.filter(
                ...
                & Q(fk_programa=settings.PROG_SL)
            ).distinct()
            context["object_list"] = object_list
            model = object_list
            if not object_list:
...
class SLDerivacionesDetailView(PermisosMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        legajo = LegajosDerivaciones.objects.filter(
            pk=pk, fk_programa=settings.PROG_SL
        ).first()
...
class SLPreAdmisionesCreateView(PermisosMixin, CreateView, SuccessMessageMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        ).annotate(cantidad_alertas=Count("fk_alerta__fk_categoria"))

        # Obtener las IDs de las categorías con alertas
        ids_categorias = [
            item["fk_alerta__fk_categoria"] for item in categorias_con_alertas
        ]

        # Obtener todas las categorías completas
        categorias_completas = CategoriaAlertas.objects.filter(
            id__in=ids_categorias
        ).order_by("nombre")
        ...
        if categorias_completas is not None:
            ...
            nombres_categorias = categorias_completas.values_list("nombre", flat=True)[
                :8
            ]
        ...
        context["alertas_alta"] = LegajoAlertas.objects.filter(
            fk_legajo=legajo_id, fk_alerta__gravedad="Critica"
        )
        context["alertas_media"] = LegajoAlertas.objects.filter(
            fk_legajo=legajo_id, fk_alerta__gravedad="Importante"
        )
        context["alertas_baja"] = LegajoAlertas.objects.filter(
            fk_legajo=legajo_id, fk_alerta__gravedad="Precaución"
        )
        context["count_alta"] = LegajoAlertas.objects.filter(
            fk_legajo=legajo_id, fk_alerta__gravedad="Critica"
        ).count()
        context["count_media"] = LegajoAlertas.objects.filter(
            fk_legajo=legajo_id, fk_alerta__gravedad="Importante"
        ).count()
        context["count_baja"] = LegajoAlertas.objects.filter(
            fk_legajo=legajo_id, fk_alerta__gravedad="Precaución"
        ).count()
        print(legajo_id)
        return context
    ...
    def form_valid(self, form):
        ...
        return HttpResponseRedirect(
            reverse("SL_preadmisiones_ver", args=[self.object.pk])
        )
...
class SLPreAdmisionesBuscarListView(PermisosMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if query:
            object_list = (
                SL_PreAdmision.objects.filter(
                    Q(fk_legajo__apellido__iexact=query)
                    | Q(fk_legajo__documento__iexact=query),
                    fk_derivacion__fk_programa_id=25,
                )
...
class SLIndiceIngresoDetailView(PermisosMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        object = SL_PreAdmision.objects.filter(pk=pk).first()
        foto_ingreso = SL_Foto_Ingreso.objects.filter(
        ...
        context["mod_puntaje"] = criterio.filter(
            fk_criterios_ingreso__modificable="SI"
...
class SLIndiceIviDetailView(PermisosMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["modificables"] = criterio.filter(
            fk_criterios_ivi__modificable="Si"
        ).count()
        context["mod_puntaje"] = criterio.filter(
            fk_criterios_ivi__modificable="Si"
...
class SLPreAdmisiones3DetailView(PermisosMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        familia = LegajoGrupoFamiliar.objects.filter(fk_legajo_2_id=legajo.fk_legajo_id)
        criterio = SL_IndiceIVI.objects.filter(fk_preadmi_id=pk, tipo="Ingreso")
        criterio_ingreso = SL_IndiceIngreso.objects.filter(
            fk_preadmi_id=pk, tipo="Ingreso"
        )
        foto_ivi = SL_Foto_IVI.objects.filter(fk_preadmi_id=pk, tipo="Ingreso").first()
        foto_ingreso = SL_Foto_Ingreso.objects.filter(
...
class SLAdmisionesListView(PermisosMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        conteo = SL_IndiceIngreso.objects.values("fk_preadmi_id").annotate(
...
class SLAdmisionesDetailView(PermisosMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        foto_ivi = SL_Foto_IVI.objects.filter(
...
class SLAsignadoAdmisionDetail(PermisosMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        observaciones = SL_Foto_IVI.objects.filter(
            fk_preadmi_id=preadmi, tipo="Ingreso"
        ).first()
        observaciones_ingreso = SL_Foto_Ingreso.objects.filter(
            fk_preadmi_id=preadmi, tipo="Ingreso"
        ).first()
        observaciones2 = SL_Foto_IVI.objects.filter(
            fk_preadmi_id=preadmi, tipo="Ingreso"
        ).first()
        intervenciones = SL_Intervenciones.objects.filter(fk_admision_id=admi.id).all()
        intervenciones_last = SL_Intervenciones.objects.filter(
            fk_admision_id=admi.id
        ).last()
        foto_ivi_fin = SL_Foto_IVI.objects.filter(
            fk_preadmi_id=admi.fk_preadmi_id, tipo="Ingreso"
        ).last()
        foto_ivi_inicio = SL_Foto_IVI.objects.filter(
            fk_preadmi_id=admi.fk_preadmi_id, tipo="Ingreso"
        ).first()
...
class SLIntervencionesCreateView(PermisosMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["object"] = SL_Admision.objects.get(
            pk=self.kwargs["pk"]
...
class SLIndiceIviEgresoCreateView(PermisosMixin, CreateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        admi = SL_Admision.objects.filter(pk=pk).first()
        # Genera una clave única utilizando uuid4 (versión aleatoria)
        preadmi = SL_PreAdmision.objects.filter(

...
def capture_old_coordinador(sender, instance, **kwargs):
    ...
    if instance.pk:
        try:
            ...
            instance._old_coordinador_id = None
    else:
        instance._old_coordinador_id = None
...
def sync_dupla_coordinador_to_profile(sender, instance, created, **kwargs):
    ...
    if not created and old_coordinador_id == (new_coordinador.id if new_coordinador else None):
        return
    ...
    if new_coordinador:
        if hasattr(new_coordinador, 'profile'):
            ...
            if not new_coordinador.profile.duplas_asignadas.filter(pk=instance.pk).exists():

...
class DuplaForm(forms.ModelForm):
    ...
    def filtrar_campos_tecnico_abogado(self):
        ...
        if grupo_tecnico:
            # Excluir usuarios que ya están asignados a una dupla, excepto el actual
            usuarios_asignados = Dupla.objects.exclude(pk=self.instance.pk).values_list("tecnico", flat=True)
            self.fields["tecnico"].queryset = grupo_tecnico.user_set.exclude(id__in=usuarios_asignados)
        else:

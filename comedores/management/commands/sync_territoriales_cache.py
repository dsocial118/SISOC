...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--comedor-id", type=int, help="ID específico de comedor para sincronizar"
        )
        parser.add_argument(
            "--force",
            action="store_true",
        ...
        parser.add_argument(
            "--stats", action="store_true", help="Mostrar estadísticas del cache actual"
        )
    ...
    def mostrar_estadisticas(self):
        ...
        self.stdout.write(
            f"Total territoriales activos: {stats.get('total_territoriales', 0)}"
        )
        self.stdout.write(
            f"Territoriales desactualizados: {stats.get('desactualizados', 0)}"
        )
        ...
        if ultimo_sync:
            ...
            self.stdout.write(
                f"Último sync exitoso: {'Sí' if stats.get('ultimo_sync_exitoso') else 'No'}"
            )
        else:
    ...
    def sincronizar_comedor(self, comedor_id, force):
        ...
        try:
            ...
            self.stdout.write(
                f"Sincronizando territoriales para comedor: {comedor.nombre}"
            )

            resultado = TerritorialService.obtener_territoriales_para_comedor(
                comedor_id=comedor_id, forzar_sync=force
            )
    ...
    def sincronizar_por_lotes(self, force):
        ...
        for comedor in comedores_muestra:
            try:
                ...
                resultado = TerritorialService.obtener_territoriales_para_comedor(
                    comedor_id=comedor.id, forzar_sync=force
                )
                ...
                if resultado["fuente"] in [
                    ...
                    self.stdout.write(
                        self.style.SUCCESS(
                            f"  ✓ {len(resultado['territoriales'])} territoriales"
                        )
                    )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Sincronización completada: {exitos} éxitos, {errores} errores"
            )
        )

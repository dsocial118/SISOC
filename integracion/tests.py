...
fortaleza de contraseña y la IP en la identidad del rate limit). Los happy-paths
y la auditoría se cubren en ``tests/test_integracion_ticketera.py``.
"""
...
@pytest.fixture
def api_key(db):
    _, key = APIKey.objects.create_key(name="integracion-tests")
    return key
...
@override_settings(INTEGRACION_TICKETERA_ENABLED=False)
def test_usuarios_responde_503_con_flag_deshabilitado(api_client):
    response = api_client.post(
        reverse("integracion-ticketera-usuarios"),
        {
            "username": "juan.perez",
...
def test_verificar_responde_503_con_flag_deshabilitado(api_client):
    response = api_client.post(
        reverse("integracion-ticketera-auth-verificar"),
...
def test_cambiar_password_responde_503_con_flag_deshabilitado(api_client):
    response = api_client.post(
        reverse("integracion-ticketera-auth-cambiar-password"),
        {
            "username": "juan.perez",
            "current_password": "x",
            "new_password": "y",
...
@pytest.mark.smoke
@pytest.mark.django_db
@override_settings(INTEGRACION_TICKETERA_ENABLED=True)
...
def test_verificar_rate_limit_identity_incluye_ip(api_client):
    with patch(
        "integracion.api_views.hit_rate_limit", return_value=False
    ) as rate_limit:
        api_client.post(
            reverse("integracion-ticketera-auth-verificar"),
            {"username": "juan.perez", "password": "x"},

...
class CupoProvinciaDetailView(View):
    def get(self, request, provincia_id: int):
        ...
        ocupados_qs = CupoService.lista_ocupados_por_provincia(
            provincia
        ...
        movimientos = (
            CupoMovimiento.objects.select_related(
                "legajo", "legajo__ciudadano", "expediente", "usuario"
            )
            .filter(
    ...
    def post(self, request, provincia_id: int):
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "Valor de cupo inválido."}, status=400
            )
...
class _BaseAccionLegajo(View):
    ...
    def _get_legajo_validado(
        self, provincia_id: int, legajo_id: int
    ) -> ExpedienteCiudadano:
        provincia = get_object_or_404(Provincia, pk=provincia_id)
...
class CupoBajaLegajoView(_BaseAccionLegajo):
    ...
    def post(self, request, provincia_id: int, legajo_id: int):
        ...
        if not form.is_valid():
            return JsonResponse(
                {"success": False, "message": form.errors.as_json()}, status=400
            )
        ...
        try:
            CupoService.liberar_slot(
                ...
                motivo=f"Baja por coordinador: {motivo}",
            )
        except CupoNoConfigurado:
            logger.warning("Baja legajo %s sin cupo configurado.", legajo.id)
        except Exception as e:
            logger.error(
                "Error al liberar cupo en baja legajo %s: %s",
                legajo.id,
                e,
                exc_info=True,
            )
            return JsonResponse(
                {"success": False, "message": "No se pudo liberar el cupo."}, status=500
            )
        ...
        return JsonResponse(
            {"success": True, "message": "Baja registrada y cupo actualizado."}
        )
    ...
    def post(self, request, provincia_id: int, legajo_id: int):
        ...
        try:
            ...
            CupoService.suspender_slot(
                legajo=legajo,
                usuario=request.user,
                motivo=f"Suspensión por coordinador: {motivo}",
            )
        except CupoNoConfigurado:
            logger.warning("Suspensión legajo %s sin cupo configurado.", legajo.id)
        except Exception as e:
            logger.error(
                "Error al suspender legajo %s: %s", legajo.id, e, exc_info=True
            )
            return JsonResponse(
                {"success": False, "message": "No se pudo suspender el legajo."},
                status=500,
            )
...
class CupoReactivarLegajoView(_BaseAccionLegajo):
    ...
    def post(self, request, provincia_id: int, legajo_id: int):
        ...
        try:
            ...
            return JsonResponse(
                {"success": False, "message": "Provincia sin cupo configurado."},
                status=400,
            )
        except Exception as e:
            logger.error(
                "Error al reactivar legajo %s: %s", legajo.id, e, exc_info=True
            )
            return JsonResponse(
                {"success": False, "message": "No se pudo reactivar el legajo."},
                status=500,
            )

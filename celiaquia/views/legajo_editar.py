...
class EditarLegajoView(View):
    def get(self, request, pk, legajo_id):
        ...
        # Verificar permisos
        if not (_is_admin(user) or _user_in_group(user, "TecnicoCeliaquia") or _user_in_group(user, "CoordinadorCeliaquia")):
            if _is_provincial(user):
                ...
                if expediente.estado.nombre not in ["CREADO", "PROCESADO", "EN_ESPERA"]:
                    return JsonResponse(
                        {"success": False, "error": "No puede editar legajos después de enviar el expediente."}, status=403
                    )
            else:
                return JsonResponse(
        ...
        data = {
            ...
            "legajo": {
                ...
                "fecha_nacimiento": ciudadano.fecha_nacimiento.strftime("%Y-%m-%d") if ciudadano.fecha_nacimiento else "",
    ...
    def post(self, request, pk, legajo_id):
        ...
        try:
            with transaction.atomic():
                ...
                if not ciudadano.apellido or not ciudadano.nombre or not ciudadano.documento:
                    raise ValidationError("Apellido, nombre y documento son obligatorios.")
                ...
                if fecha_str:
                    ...
                    try:
                        ciudadano.fecha_nacimiento = datetime.strptime(fecha_str, "%Y-%m-%d").date()
                    except ValueError:
                ...
                if nacionalidad_id:
                    try:
                        ciudadano.nacionalidad = Nacionalidad.objects.get(pk=nacionalidad_id)
                ...
                ciudadano.codigo_postal = request.POST.get("codigo_postal", "").strip() or None
            ...
            return JsonResponse(
                {"success": False, "error": "Error interno al actualizar los datos."}, status=500
            )
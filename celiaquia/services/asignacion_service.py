...
class AsignacionService:
    ...
    @transaction.atomic
    def asignar_tecnico(
        expediente: Expediente, tecnico: User, usuario_asignador: User = None
    ):
        """
        ...
        AsignacionTecnico.objects.filter(expediente=expediente, activa=True).update(
            activa=False
        )

        # Crear nueva asignación
        asignacion = AsignacionTecnico.objects.create(
            expediente=expediente, tecnico=tecnico, activa=True
        )
    ...
    def obtener_tecnico_activo(expediente: Expediente):
        ...
        try:
            asignacion = AsignacionTecnico.objects.get(
                expediente=expediente, activa=True
            )
    ...
    def obtener_historial_asignaciones(expediente: Expediente):
        ...
        return (
            AsignacionTecnico.objects.filter(expediente=expediente)

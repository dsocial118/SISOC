...
class PadronFinalService:
    ...
    def generar_padron_final_excel(expediente) -> bytes:
        ...
        legajos = ExpedienteCiudadano.objects.filter(
            expediente=expediente
        ).select_related('ciudadano', 'estado').exclude(
        ...
        for legajo in legajos:
            ...
            registro = {
                ...
                'FechaNacimiento': ciudadano.fecha_nacimiento.strftime('%d/%m/%Y') if ciudadano.fecha_nacimiento else '',
            ...
            if tipo_registro == "Beneficiario":
                ...
                if responsable_legajo:
                    ...
                    registro['ResponsableNombre'] = f"{resp_ciudadano.apellido} {resp_ciudadano.nombre}".strip()
        ...
        # Orden de columnas
        columnas = [
            'TipoRegistro', 'Apellido', 'Nombre', 'Documento', 'CUIL_CUIT',
            'FechaNacimiento', 'Sexo', 'Provincia', 'Municipio', 'Localidad',
            'ExpedienteID', 'EstadoLegajo', 'RolLegajo', 'ResponsableDocumento', 'ResponsableNombre'
        ]
        df = df[columnas]

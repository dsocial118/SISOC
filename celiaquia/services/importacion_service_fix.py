...
for leg in legajos:
    print(
        f"  - {leg.ciudadano.nombre} {leg.ciudadano.apellido} (DNI: {leg.ciudadano.documento}) - Rol: {leg.rol}"
    )
...
) | GrupoFamiliar.objects.filter(ciudadano_2__expedientes__expediente=expediente)

for rel in relaciones:
    print(
        f"  - {rel.ciudadano_1.nombre} {rel.ciudadano_1.apellido} ({rel.vinculo}) {rel.ciudadano_2.nombre} {rel.ciudadano_2.apellido}"
    )

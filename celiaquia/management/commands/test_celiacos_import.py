...
class Command(BaseCommand):
        ...
        try:
            ...
            if legajos.exists():
                ...
                for legajo in legajos:
                    self.print_info(
                        f"  - {legajo.apellido}, {legajo.nombre} (ID: {legajo.id})"
                    )
    ...
    def test_caso_c(self, user):
        ...
        try:
            ...
            legajos = ExpedienteCiudadano.objects.filter(
                apellido__in=["García", "López", "Rodríguez"]
            )
            if legajos.count() >= 3:
                self.print_ok(f"✓ {legajos.count()} expediente(s) creado(s) en BD")
                for legajo in legajos:
                ...
                self.print_error(
                    f"Se esperaban 3 expedientes, se crearon {legajos.count()}"
                )
                return False
    ...
    def test_caso_e(self, user):
        ...
        try:
            ...
            if legajos.count() >= 2:
                ...
                self.print_error(
                    f"Se esperaban 2 expedientes, se crearon {legajos.count()}"
                )
                return False
    ...
    def print_resumen(self, resultados):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"✓ DATOS INSERTADOS EN LA BASE DE DATOS USANDO PROCESO REAL"
            )
        )
        self.stdout.write("=" * 80 + "\n")

...
class Command(BaseCommand):
    ...
    def test_caso_a(self, user):
        ...
        try:
            with transaction.atomic():
                ...
                self.print_info(f'✓ Creado: {legajo.apellido}, {legajo.nombre} (ID: {legajo.id})')
    ...
    def test_caso_b(self, user):
        ...
        try:
            with transaction.atomic():
                ...
                self.print_info(f'✓ Responsable: {responsable.apellido}, {responsable.nombre} (ID: {responsable.id})')
                ...
                self.print_info(f'✓ Beneficiario: {beneficiario.apellido}, {beneficiario.nombre} (ID: {beneficiario.id})')
                
                ComentariosService.agregar_subsanacion_motivo(
                    legajo=beneficiario,
                    motivo='Falta certificado médico',
                    usuario=user
                )
    ...
    def test_caso_e(self, user):
        ...
        try:
            with transaction.atomic():
                ...
                self.print_info(f'✓ Responsable menor: {responsable.apellido}, {responsable.nombre} (ID: {responsable.id})')
    ...
    def test_caso_f(self, user):
        ...
        try:
            with transaction.atomic():
                ...
                self.print_info(f'✓ Abuelo: {abuelo.apellido}, {abuelo.nombre} (ID: {abuelo.id})')
                ...
                self.print_info(f'✓ Padre: {padre.apellido}, {padre.nombre} (ID: {padre.id})')
                ...
                self.print_info(f'✓ Hijo: {hijo.apellido}, {hijo.nombre} (ID: {hijo.id})')
    ...
    def test_error_1(self, user):
        ...
        try:
            ...
            edad_beneficiario = (datetime.now().date() - beneficiario_fecha).days / 365.25

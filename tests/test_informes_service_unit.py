...
def test_generar_y_guardar_pdf_and_context_helpers(mocker):
    ...
    informe = SimpleNamespace(
        ...
        pk=10,
        tipo="base",
        admision=SimpleNamespace(id=1, comedor=SimpleNamespace(nombre="C"), tipo="incorporacion"),
    )

    mocker.patch("admisiones.services.informes_service.generar_texto_comidas", return_value="txt")
    mocker.patch("admisiones.services.informes_service.render_to_string", side_effect=["<html>pdf</html>", "<html>docx</html>"])
    mocker.patch("admisiones.services.informes_service.HTML", return_value=SimpleNamespace(write_pdf=lambda: b"pdf"))
    mocker.patch.object(module.InformeService, "generar_docx_con_template", return_value=BytesIO(b"docx"))
    upd = mocker.patch("admisiones.services.informes_service.InformeTecnicoPDF.objects.update_or_create", return_value=(SimpleNamespace(pk=1), True))
    ...
    mocker.patch("admisiones.services.informes_service.CampoASubsanar.objects.filter", return_value=SimpleNamespace(values_list=lambda *a, **k: ["campo_x"]))
    mocker.patch("admisiones.services.informes_service.ObservacionGeneralInforme.DoesNotExist", obs_exc)
    mocker.patch("admisiones.services.informes_service.ObservacionGeneralInforme.objects.get", side_effect=obs_exc())
    informe2 = SimpleNamespace(
        admision=SimpleNamespace(comedor=SimpleNamespace()),
        _meta=SimpleNamespace(fields=[SimpleNamespace(name="campo_x", verbose_name="Campo X")]),
    )
    mocker.patch.object(module.InformeService, "get_campos_visibles_informe", return_value=[("Campo X", "v")])
...
def test_guardar_informe_and_detail_context(mocker):
    ...
    instance = SimpleNamespace(tipo="base", pk=None, _state=SimpleNamespace(adding=True), estado_formulario="finalizado", observaciones_subsanacion="obs", admision=None, save=mocker.Mock())
    form = mocker.Mock(instance=instance, save=mocker.Mock(return_value=instance), save_m2m=mocker.Mock())

    mocker.patch("admisiones.services.informes_service.InformeTecnico.objects.filter", return_value=SimpleNamespace(order_by=lambda *_: SimpleNamespace(first=lambda: None)))
    mocker.patch.object(module.InformeService, "generar_docx_borrador", return_value=SimpleNamespace())
    mocker.patch("admisiones.services.admisiones_service.AdmisionService.actualizar_estado_admision")

    res = module.InformeService.guardar_informe(form, admision, es_creacion=True, action="submit", usuario=SimpleNamespace())
    assert res["success"] is True

    informe_det = SimpleNamespace(admision=admision, estado="Validado", estado_formulario="finalizado", id=5)
    mocker.patch("admisiones.services.informes_service.InformeTecnicoPDF.objects.filter", return_value=SimpleNamespace(first=lambda: "pdf"))
    mocker.patch.object(module.InformeService, "get_campos_visibles_informe", return_value=[])
...
def test_revision_and_complementarios_flows(mocker):
    ...
    req = SimpleNamespace(POST=SimpleNamespace(get=lambda k, d=None: {"estado": "A subsanar", "observacion": "obs"}.get(k, d), getlist=lambda k: ["Campo"]))
    upd = mocker.patch.object(module.InformeService, "actualizar_estado_informe")
    mocker.patch("admisiones.services.informes_service.CampoASubsanar.objects.filter", return_value=SimpleNamespace(delete=mocker.Mock()))
    mocker.patch("admisiones.services.informes_service.CampoASubsanar.objects.create")
    mocker.patch("admisiones.services.informes_service.ObservacionGeneralInforme.objects.get_or_create", return_value=(SimpleNamespace(texto="", save=mocker.Mock()), True))
    module.InformeService.procesar_revision_informe(req, "base", informe)
    assert upd.called

    inf_comp = SimpleNamespace(pk=1, admision=informe.admision, informe_tecnico=informe, creado_por=SimpleNamespace(username="u"), creado=None)
    mocker.patch("admisiones.services.informes_service.InformeComplementario.objects.get_or_create", return_value=(inf_comp, True))
    mocker.patch("admisiones.services.informes_service.InformeComplementarioCampos.objects.filter", return_value=SimpleNamespace(delete=mocker.Mock()))
    create = mocker.patch("admisiones.services.informes_service.InformeComplementarioCampos.objects.create")
    saved = module.InformeService.guardar_campos_complementarios(informe, {"campo": "valor"}, usuario=SimpleNamespace())
    assert saved is inf_comp
    assert create.called

    campo_obj = SimpleNamespace(campo="campo", value="nuevo")
    mocker.patch("admisiones.services.informes_service.InformeComplementarioCampos.objects.filter", return_value=[campo_obj])
    mocker.patch("admisiones.services.informes_service.generar_texto_comidas", return_value="txt")
    mocker.patch("admisiones.services.informes_service.render_to_string", return_value="<html></html>")
    mocker.patch("admisiones.services.informes_service.HTML", return_value=SimpleNamespace(write_pdf=lambda: b"pdf"))
    mocker.patch("admisiones.services.informes_service.DocumentTemplateService.generar_docx", return_value=BytesIO(b"docx"))
    mocker.patch("admisiones.services.informes_service.InformeTecnicoComplementarioPDF.objects.update_or_create")
    assert module.InformeService.generar_y_guardar_pdf_complementario(inf_comp) is True

    mocker.patch.object(module.InformeService, "generar_docx_con_template", return_value=BytesIO(b"docx"))
    mocker.patch("admisiones.services.informes_service.InformeTecnicoPDF.objects.update_or_create", return_value=(SimpleNamespace(pk=1), True))
    borrador = module.InformeService.generar_docx_borrador(SimpleNamespace(id=4, pk=4, tipo="base", estado="Iniciado", admision=SimpleNamespace(id=4, comedor=SimpleNamespace()), admision_id=4, save=mocker.Mock()))
    assert borrador is not None

    pdf_obj = SimpleNamespace(save=mocker.Mock())
    mocker.patch("admisiones.services.informes_service.InformeTecnicoPDF.objects.get_or_create", return_value=(pdf_obj, False))
    mocker.patch("admisiones.services.admisiones_service.AdmisionService.actualizar_estado_admision")
    informe_upload = SimpleNamespace(id=8, pk=8, tipo="base", estado="x", admision=SimpleNamespace(id=8, comedor=SimpleNamespace()), admision_id=8, save=mocker.Mock())
    archivo = SimpleNamespace(name="x")
    assert module.InformeService.subir_docx_editado(informe_upload, archivo) is pdf_obj

    mocker.patch("admisiones.services.informes_service.InformeComplementarioCampos.objects.filter", return_value=[SimpleNamespace()])

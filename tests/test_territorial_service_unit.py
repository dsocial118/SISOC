...
def test_basic_helpers_parse_and_cache_update():
    assert (
        module.TerritorialService._get_cache_key_provincia(5)
        == "territoriales_provincia_5"
    )

    parsed = module.TerritorialService._parse_territoriales_string(
        "u1/Uno, u2 / Dos , invalido"
    )
    assert parsed == [
    ...
    assert (
        module.TerritorialService._actualizar_cache_db(
            [
                {"gestionar_uid": "a", "nombre": "A"},
                {"gestionar_uid": "b", "nombre": "B"},
            ]
        )
        == 2
    )
...
def test_sync_calls_and_errors(mocker):
    ...
    mocker.patch.object(
        module.TerritorialService, "_actualizar_cache_db", return_value=1
    )
    ...
    mocker.patch.object(
        module.TerritorialService, "_actualizar_cache_db_provincia", return_value=1
    )
    mocker.patch.object(
        module,
        "cache",

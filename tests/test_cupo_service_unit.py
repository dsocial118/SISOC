...
def test_metrics_and_list_queries(mocker):
    ...
    mocker.patch(
        "celiaquia.services.cupo_service.ProvinciaCupo.objects.only",
        return_value=_Chain(pc),
    )
    mocker.patch(
        "celiaquia.services.cupo_service.ExpedienteCiudadano.objects.filter",
        return_value=SimpleNamespace(count=lambda: 2),
    )
...
def test_metrics_no_config_raises(mocker):
    ...
    mocker.patch(
        "celiaquia.services.cupo_service.ProvinciaCupo.objects.only",
        return_value=SimpleNamespace(get=mocker.Mock(side_effect=dne())),
    )
...
def test_configurar_total_create_update_and_validation(mocker):
    ...
    mocker.patch(
        "celiaquia.services.cupo_service.ProvinciaCupo.objects.get_or_create",
        return_value=(pc, True),
    )
...
def test_reservar_slot_early_branches(mocker):
    ...
    mocker.patch(
        "celiaquia.services.cupo_service.ExpedienteCiudadano.objects.select_for_update",
        return_value=_Chain(leg),
    )
...
def test_suspender_liberar_reactivar_basic_branches(mocker):
    provincia = "PBA"
    exp = SimpleNamespace(
        usuario_provincia=SimpleNamespace(profile=SimpleNamespace(provincia=provincia))
    )
    ...
    mocker.patch(
        "celiaquia.services.cupo_service.ExpedienteCiudadano.objects.select_for_update",
        return_value=_Chain(leg_l),
    )
    mocker.patch(
        "celiaquia.services.cupo_service.ProvinciaCupo.objects.select_for_update",
        return_value=_Chain(pc),
    )
    mocker.patch(
        "celiaquia.services.cupo_service.ProvinciaCupo.objects.filter",
        return_value=SimpleNamespace(update=mocker.Mock()),
    )
    mocker.patch("celiaquia.services.cupo_service.HistorialCupo.objects.create")
    mocker.patch("celiaquia.services.cupo_service.CupoMovimiento.objects.create")

...
usage() {
  cat <<'USAGE'
Uso:
  restart_hml.sh [--apply --acknowledge-db-writes]

Sin flags muestra el plan. El restart real reconstruye backend/mobile sin pull
y puede escribir DB; requiere aprobacion operativa explicita.
USAGE
}
...
if [[ "$APPLY" -eq 0 ]]; then
  printf 'Plan: backup, rebuild sin pull de backend/mobile y health check.\n'

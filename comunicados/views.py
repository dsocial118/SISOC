...
class ComunicadoGestionListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if es_tecnico(user) and not is_admin(user):
            comedores_ids = get_ids_comedores_del_usuario(user)
            organizaciones_ids = get_ids_organizaciones_del_usuario(user)
            queryset = (
                queryset.filter(tipo=TipoComunicado.EXTERNO)
                .filter(
                    Q(comedores__pk__in=comedores_ids)
                    | Q(organizaciones__pk__in=organizaciones_ids)
                )
                .distinct()
            )
        ...
        if self.object.tipo == TipoComunicado.EXTERNO:
            if self.object.subtipo == SubtipoComunicado.INSTITUCIONAL:
                ...
                if form.cleaned_data.get("para_todos_comedores"):
                    comedor_ids = get_ids_comedores_del_usuario(self.request.user)
                    self.object.comedores.set(comedor_ids)
                else:

...
data_df["FECHA_NACIMIENTO"] = pd.to_datetime(
    data_df["FECHA_NACIMIENTO"], format="%d.%m.%Y"
)
data_df["COD_FALLECIMIENTO"] = pd.to_numeric(
    data_df["COD_FALLECIMIENTO"], errors="coerce"
)
data_df["FECHA_FALLECIMIENTO"] = pd.to_datetime(
    data_df["FECHA_FALLECIMIENTO"], format="%d.%m.%Y"
)
...
data_df["APELLIDO"], data_df["NONMBRE"] = zip(
    *data_df["APELLIDO_NOMBRE"].apply(split_name_fullname)
)

data_df["FECHA_NACIMIENTO2"] = pd.to_datetime(
    data_df["FECHA_NACIMIENTO2"], format="%d.%m.%Y"
)
data_df["COD_FALLECIMIENTO2"] = pd.to_numeric(
    data_df["COD_FALLECIMIENTO2"], errors="coerce"
)
data_df["FECHA_FALLECIMIENTO2"] = pd.to_datetime(
    data_df["FECHA_FALLECIMIENTO2"], format="%d.%m.%Y"
)
...
# ssh_host = '10.80.9.15'
# ssh_port = 22
# ssh_username = 'REDACTED_DB_USER'
# ssh_password = 'REDACTED_DB_PASSWORD'
...
# Create the engine
# engine = create_engine('mysql+mysqlconnector://REDACTED_DB_USER:REDACTED_DB_PASSWORD@10.80.9.15:3306/pas_padron_01_05_2024', connect_args={'connect_timeout': 288000}, pool_pre_ping=True)
...
engine = create_engine(
    connection_string, connect_args={"connect_timeout": 288000}, pool_pre_ping=True
)

...
| Relevamiento | Registro de visita/encuesta al comedor y ancla del ciclo de seguimiento. | `comedor`, `estado`, `fecha_visita`, `territorial_uid/nombre` externos, `territorial_user` opcional, `origen`, `asignado_desde_sisoc`, `client_uuid` y componentes OneToOne. Evidencia: relevamientos/models.py. | Único activo por comedor; `client_uuid` hace idempotentes los reintentos territoriales. Los UID externos heredados no se convierten automáticamente en usuarios SISOC. El ciclo de seguimientos conserva orden, revisión del coordinador y actas extraordinarias. Evidencia: relevamientos/models.py, migrations 0012 y 0014-0017. |

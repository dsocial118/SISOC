...
- Colocar opcionalmente un dump en `docker/mysql/local-dump.sql`, luego levantar servicios con `docker compose up` y acceder en `http://localhost:8001` (valor por defecto de `DOCKER_DJANGO_PORT_FORWARD` en `.env.example`). Evidencia: README.md y .env.example.
...
## Debug y desarrollo
- Debug recomendado en VSCode con la configuración “Django in Docker”; levantar servicios antes con `docker compose up`. Evidencia: README.md.

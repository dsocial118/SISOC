...
- Tipo de cambio: Evolutivo funcional (incluye dos correcciones detectadas durante la implementación).
- Área principal: celiaquia — detalle de expediente, revisión de legajos.
- Contexto funcional: Celiaquía — revisión técnica de legajos. El técnico registra observaciones estructuradas y esas mismas observaciones son las que se le comunican a la Provincia al subsanar o rechazar, sin volver a redactarlas.
- Resumen para changelog: Los comentarios técnicos del legajo pasan a ser estructurados (tipo de documento, si tiene observaciones y observación de un catálogo) y alimentan automáticamente el motivo de Subsanar y Rechazar, que dejan de pedir un texto aparte. La Provincia recibe únicamente las observaciones marcadas con "Sí", y sólo al solicitarse la subsanación o el rechazo.
- Impacto usuario: Técnico y Coordinador dejan de reescribir motivos y cargan observaciones de un catálogo cerrado. La Provincia recibe un texto uniforme y trazable. El comentario de texto libre y el adjunto se retiran del formulario (el requerimiento enumera cuatro campos y ninguno es texto suelto); el endpoint los sigue aceptando, así que nada externo se rompe.

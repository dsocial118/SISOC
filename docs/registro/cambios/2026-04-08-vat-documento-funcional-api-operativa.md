...
Luego se amplió para documentar explícitamente el comportamiento de paginación en respuestas de listados.
...
- Explica cómo interpretar `count`, `results`, `next` y `previous` al consumir listados paginados.
...
- La documentación de paginación se apoya en el comportamiento real observado del endpoint `GET /api/vat/centros/` en producción.
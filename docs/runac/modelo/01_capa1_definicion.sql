...
CREATE TABLE `mir_c1_archivo_version` (
  `id` bigint PRIMARY KEY AUTO_INCREMENT COMMENT 'Identificador interno.',
  ...
  UNIQUE KEY `mir_c1_archivo_version_unica` (`archivo_id`, `numero`)
...
CREATE TABLE `mir_c1_campo` (
  ...
  `tipo_dato` ENUM ('TEXTO', 'ENTERO', 'DECIMAL', 'FECHA', 'HORA') NOT NULL COMMENT 'Tipo de dato esperado para el campo. HORA se incorporó al detectar campos de horario que se estaban guardando como fecha.',
...
CREATE TABLE `mir_c1_catalogo_opcion` (
  `id` bigint PRIMARY KEY AUTO_INCREMENT COMMENT 'Identificador interno.',
...
CREATE TABLE `mir_c1_tipo_regla` (
  `id` bigint PRIMARY KEY AUTO_INCREMENT COMMENT 'Identificador interno.',
  `nombre` varchar(100) UNIQUE NOT NULL COMMENT 'Nombre técnico y estable del tipo de regla, por ejemplo RANGO, OBLIGATORIO_SI, EXISTE_EN_ARCHIVO o EJECUTAR_FUNCION.',
...
CREATE TABLE `mir_c1_tipo_regla_parametro` (
  `id` bigint PRIMARY KEY AUTO_INCREMENT COMMENT 'Identificador interno.',
...
CREATE TABLE `mir_c1_campo_regla` (
  `id` bigint PRIMARY KEY AUTO_INCREMENT COMMENT 'Identificador interno.',
...
CREATE UNIQUE INDEX `mir_c1_hoja_index_0` ON `mir_c1_hoja` (`archivo_version_id`, `nombre_esperado`);
CREATE UNIQUE INDEX `mir_c1_hoja_index_1` ON `mir_c1_hoja` (`archivo_version_id`, `orden_procesamiento`);
CREATE UNIQUE INDEX `mir_c1_dimension_index_2` ON `mir_c1_dimension` (`hoja_id`, `orden`);
CREATE UNIQUE INDEX `mir_c1_campo_index_3` ON `mir_c1_campo` (`hoja_id`, `nombre`);
CREATE UNIQUE INDEX `mir_c1_campo_index_4` ON `mir_c1_campo` (`hoja_id`, `orden`);
CREATE UNIQUE INDEX `mir_c1_catalogo_opcion_index_5` ON `mir_c1_catalogo_opcion` (`catalogo_id`, `codigo`);
CREATE UNIQUE INDEX `mir_c1_catalogo_opcion_index_6` ON `mir_c1_catalogo_opcion` (`catalogo_id`, `valor_esperado`);
CREATE UNIQUE INDEX `mir_c1_catalogo_opcion_index_7` ON `mir_c1_catalogo_opcion` (`catalogo_id`, `orden`);
CREATE UNIQUE INDEX `mir_c1_tipo_regla_parametro_index_8` ON `mir_c1_tipo_regla_parametro` (`tipo_regla_id`, `nombre`);
CREATE UNIQUE INDEX `mir_c1_tipo_regla_parametro_index_9` ON `mir_c1_tipo_regla_parametro` (`tipo_regla_id`, `orden`);
CREATE UNIQUE INDEX `mir_c1_campo_regla_index_10` ON `mir_c1_campo_regla` (`campo_id`, `regla_id`);
CREATE INDEX `mir_c1_archivo_version_estado` ON `mir_c1_archivo_version` (`archivo_id`, `estado`);

-- ---------------------------------------------------------------------------
-- Claves foráneas
-- ---------------------------------------------------------------------------

ALTER TABLE `mir_c1_archivo_version` ADD FOREIGN KEY (`archivo_id`) REFERENCES `mir_c1_archivo` (`id`);
ALTER TABLE `mir_c1_archivo_version` ADD FOREIGN KEY (`copiada_de`) REFERENCES `mir_c1_archivo_version` (`id`);
ALTER TABLE `mir_c1_hoja` ADD FOREIGN KEY (`archivo_version_id`) REFERENCES `mir_c1_archivo_version` (`id`);
ALTER TABLE `mir_c1_dimension` ADD FOREIGN KEY (`hoja_id`) REFERENCES `mir_c1_hoja` (`id`);
ALTER TABLE `mir_c1_campo` ADD FOREIGN KEY (`hoja_id`) REFERENCES `mir_c1_hoja` (`id`);
ALTER TABLE `mir_c1_campo` ADD FOREIGN KEY (`dimension_id`) REFERENCES `mir_c1_dimension` (`id`);
ALTER TABLE `mir_c1_campo` ADD FOREIGN KEY (`catalogo_id`) REFERENCES `mir_c1_catalogo` (`id`);
ALTER TABLE `mir_c1_catalogo_opcion` ADD FOREIGN KEY (`catalogo_id`) REFERENCES `mir_c1_catalogo` (`id`);
ALTER TABLE `mir_c1_tipo_regla_parametro` ADD FOREIGN KEY (`tipo_regla_id`) REFERENCES `mir_c1_tipo_regla` (`id`);
ALTER TABLE `mir_c1_regla` ADD FOREIGN KEY (`tipo_regla_id`) REFERENCES `mir_c1_tipo_regla` (`id`);
ALTER TABLE `mir_c1_campo_regla` ADD FOREIGN KEY (`campo_id`) REFERENCES `mir_c1_campo` (`id`);
ALTER TABLE `mir_c1_campo_regla` ADD FOREIGN KEY (`regla_id`) REFERENCES `mir_c1_regla` (`id`);

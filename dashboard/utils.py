...
def calcular_presupuesto_desayuno():
    ...
    cached_value = cache.get(cache_key)
    if cached_value is None:
        cached_value = ValorComida.objects.filter(tipo="desayuno").aggregate(total=Sum('valor'))['total'] or 0
        cache.set(cache_key, cached_value, timeout=CACHE_TIMEOUT)
    return cached_value
...
def calcular_presupuesto_merienda():
    ...
    cached_value = cache.get(cache_key)
    if cached_value is None:
        cached_value = ValorComida.objects.filter(tipo="merienda").aggregate(total=Sum('valor'))['total'] or 0
        cache.set(cache_key, cached_value, timeout=CACHE_TIMEOUT)
    return cached_value
...
def calcular_presupuesto_comida():
    ...
    cached_value = cache.get(cache_key)
    if cached_value is None:
        cached_value = ValorComida.objects.filter(tipo="comida").aggregate(total=Sum('valor'))['total'] or 0
        cache.set(cache_key, cached_value, timeout=CACHE_TIMEOUT)
    return cached_value

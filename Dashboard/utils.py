...
def obtener_alarmas_activas():
    ...
    if alarmas_activas is None:
        alarmas_activas = list(Alertas.objects.filter(gravedad='Critica').values_list('id'))
        cache.set('alarmas_activas', alarmas_activas, 60 * 15)  # Almacenar en caché por 15 minutos
...
def obtener_legajos_counts():
    ...
    if legajos_counts is None:
        ...
        cache.set('legajos_counts', legajos_counts, 60 * 15)  # Almacenar en caché por 15 minutos
...
def obtener_alertas_embarazo_ids():
    ...
    if alertas_embarazo_ids is None:
        alertas_embarazo_ids = list(Alertas.objects.filter(fk_categoria__nombre__istartswith='embarazo').values_list('id', flat=True))
        cache.set('alertas_embarazo_ids', alertas_embarazo_ids, 60 * 15)  # Almacenar en caché por 15 minutos
...
def obtener_legajos_con_alerta_embarazo():
    ...
    if legajos_con_alerta_embarazo is None:
        ...
        cache.set('legajos_con_alerta_embarazo', legajos_con_alerta_embarazo, 60 * 15)  # Almacenar en caché por 15 minutos
...
def contar_embarazos_sin_derivacion_aceptada():
    legajos_con_alerta_embarazo = obtener_legajos_con_alerta_embarazo()
    return legajos_con_alerta_embarazo.exclude(legajosderivaciones__estado='Aceptada').count()

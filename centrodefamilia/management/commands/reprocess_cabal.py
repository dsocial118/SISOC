...
class Command(BaseCommand):
    ...
    def _double_confirm(self) -> bool:
        q1 = (
            input("Esto modificará datos. Escribí 'Y' para confirmar: ").strip().upper()
        )
        if q1 != "Y":
            return False
        q2 = (
            input("Confirmación final: escribí nuevamente 'Y' para ejecutar: ")
            .strip()
            .upper()
        )
        return q2 == "Y"
    ...
    def handle(self, *args, **opts):
        ...
        if do_commit:
            if not sys.stdin.isatty():
                raise CommandError(
                    "Ejecución no interactiva: no se puede confirmar. Corré sin --commit para dry-run."
                )

...
class CentroForm(forms.ModelForm):
    class Meta:
        model = Centro
        fields = ['nombre', 'direccion', 'contacto', 'tipo', 'activo', 'faro_asociado', 'referente']
...
class ActividadCentroForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            'actividad', 'cantidad_personas',
            'dias', 'horarios', 'estado'
        ]
        exclude = ['centro']
...
class ParticipanteActividadForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['actividad_centro', 'nombre', 'apellido', 'dni', 'edad', 'genero', 'cuit']
    ...
    def clean(self):
        ...
        if actividad and cuit:
            if ParticipanteActividad.objects.filter(actividad_centro=actividad, cuit=cuit).exists():
                raise ValidationError("Este CUIT ya está registrado para esta actividad.")
